...
contract GlobalToken{
    ...
    function transfer(address to, uint256 amount) public returns (bool) {
        ...
        if(_users[sender] && _tps[to]){
            //transfer only can occure from user to tp
            _transferUserToTP(sender, to, amount);
            return true;  
        }
        if(_admins[sender] && _users[to]){
            //admin transfers tokens to an user
            //Darle otra vuelta a esto, quizás es mejor hacer un allowance para mintear¿?
            _mint(to, amount);
            return true;  
        }
